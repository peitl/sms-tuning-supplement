...
#define SMALL_CONST 1.0E-10

inline double min( double a, double b ) { return a < b ? a : b; }

inline double diffScoringFunction0(double x, double y)
{
  return x + y + x*y;
}

inline double diffScoringFunction1(double x, double y)
{
  return min(x,y) + SMALL_CONST * (x + y);
}

inline double diffScoringFunction2(double x, double y)
{
  return x + y;
}
  ...
  do {
	...
	if ((only_reduced_variables_flag == 1) &&
       ...
       if (IS_NOT_FORCED(varnr)) {
        ...
        {
        case 0:
          diffScore = diffScoringFunction0(left, right);
          break;

        case 1:
          diffScore = diffScoringFunction1(left, right);
          break;

        case 2:
          diffScore = diffScoringFunction2(left, right);
          break;
        
        default:
          printf("Error: using unknown scoring function\n");
          exit(1);
          break;
        }
