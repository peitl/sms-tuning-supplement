...
void printCube(const vector<vector<int>> &cubeTrail, const vector<int> &, bool onlyDecisions = true)
{
    if (!onlyDecisions)
    {
        printf("a");
...
inline double scoringFunction(const std::pair<int, int> &scores)
{
    return std::min(scores.first, scores.second) + 0.000000001 * (scores.first + scores.second);
}

// returns the k best variables
inline vector<int> getKBestVars(const vector<std::pair<int, int>> &scores, int k)
{
    vector<int> bestVars;
    vector<std::pair<int, double>> scoresWithValues;
    for (int i = 1; i < (int)scores.size(); i++)
    {
        if (scores[i].first == 0 && scores[i].second == 0)
            continue;
        scoresWithValues.push_back({i, scoringFunction(scores[i])});
    }

    std::sort(scoresWithValues.begin(), scoresWithValues.end(), [](const std::pair<int, double> &a, const std::pair<int, double> &b)
              { return a.second > b.second; });

    for (int i = 0; i < k && i < (int)scoresWithValues.size(); i++)
        bestVars.push_back(scoresWithValues[i].first);

    return bestVars;
}

inline int getBestVar(const vector<std::pair<int, int>> &scores)
{
    double bestScore = -1;
    int bestVar = 0;
    for (int i = 1; i < (int)scores.size(); i++)
...
bool GraphSolver::checkCutoff(const vector<vector<int>> &cubeTrail, int assignmentCutoff)
{
    if (countAssignedOrInactive(cubeTrail) >= assignmentCutoff)
    {
        ...
        this->stats.numCubes++;
        if (config.maxNumCubes && this->stats.numCubes > config.maxNumCubes)
        {
...
void GraphSolver::generateCubesRec(cubing_state_t &cubingState, int assignmentCutoff)
{
    ...
    else if (useLookahead)
    {
        ...
        branchingVar = getBestVar(scores);
        assert (branchingVar > 0);
        std::pair<int, int> varScore = scores[abs(branchingVar)];
...
bool GraphSolver::createGameRec(cubing_state_t &cubingState, int recDepth, int &selectedBranchingLit, int &finalScore)
{
    ...
    int branchingLit = getBestVar(scores);
    assert (branchingLit > 0);
    std::pair<int, int> varScore = scores[abs(branchingLit)];
