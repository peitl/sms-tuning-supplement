...
void initOptions(SolverConfig &config, struct minimality_config_t &minimalityConfig, propagators_config_t &propagatorsConfig)
{
    ...
    other_opts.add_options()
      ...
      ("max-num-cubes", po::value<int>(&config.maxNumCubes), "An upperbound on the number of cubes, if exceed abort with exit code 25")
      ...
      ("sample-propagator",  po::bool_switch(&propagatorsConfig.samplePropagator), "Example of a simple propagator")
      ("learned-clauses-file-test", po::value<string>(&config.learnedClausesFileTest), "Test whether the given clauses preserve all models")
      ;
    // ------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
#include "graphPropagators/samplePropagator.hpp"
...
void addPropagators(GraphSolver *solver, const propagators_config_t &propagatorsConfig)
{
    ...
    if (propagatorsConfig.samplePropagator)
        solver->addPartiallyDefinedGraphChecker(new SamplePropagator());
    ...
    if (propagatorsConfig.forbiddenSubgraphFile != "" || propagatorsConfig.forbiddenInducedSubgraphFile != "")
    {
        ...
        if (propagatorsConfig.forbiddenSubgraphFile != "")
        {
            ...
            if (!forbiddenSubgraphFile.is_open())
                throw std::runtime_error("Could not open forbidden subgraph file");
        }
        if (propagatorsConfig.forbiddenInducedSubgraphFile != "")
        {
            ...
            if (!forbiddenInducedSubgraphFile.is_open())
                throw std::runtime_error("Could not open forbidden induced subgraph file");
        }
...
GraphSolver *createSolver(SolverConfig &config, struct minimality_config_t &minimalitConfig, propagators_config_t &propagatorsConfig, string dimacsFile)
{
    ...
    if (!config.learnedClausesFileTest.empty())
    {
        std::ifstream learnedClausesFile(config.learnedClausesFileTest);
        if (!learnedClausesFile.is_open())
        {
            throw std::runtime_error("Could not open learned clauses file");
        }
        clause_t oneClauseFalse;
        std::string line;
        while (std::getline(learnedClausesFile, line))
        {
            // skip lines starts with "c " then skip
            ...
            while (iss >> lit)
            {
                ...
                if (litInt == 0)
                    break; // end of clause
                clause.push_back(litInt);
            }

            solver->numVars++;
            int clauseIsFalse = solver->numVars;
            oneClauseFalse.push_back(clauseIsFalse);

            // if clause is false then all literals are assigned to the opposite
            for (auto lit : clause)
            {
                solver->add(-clauseIsFalse);
                solver->add(-lit);
                solver->add(0);
            }
        }
