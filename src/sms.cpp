...
public:
  ...
  CubeReader(const std::string &filename, const vector<int> range, int cubeLine)
  {
    ...
    {
      // check if it is line with fixed literals, i.e., starting with 'c Fixed literals:'
      if (line.substr(0, 17) == "c Fixed literals:")
      {
        line = line.substr(17);
        std::istringstream iss(line);
        std::string lit;
        while (iss >> lit)
          fixedLiterals.push_back(std::stoi(lit));
      }
      currentLine++;
...
int GraphSolver::sms_solve()
{
  ...
  {
    CubeReader r(config.cubeFile, config.cubesRange, config.cubeLine);
    printf("Number of fixed literals: %d\n", (int)r.fixedLiterals.size());
    for (int lit : r.fixedLiterals)
    {
      this->add(lit);
      this->add(0);
    }
